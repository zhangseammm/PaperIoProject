...
def play(stat, storage):
    '''
    通过关键字定义，方便接下来查询
    这个函数每一回合就会执行一次，而load只会在第一次执行，所以direction/x/y/field/band需要每一回合更新一次
    '''
    ...
    def turnDir(size,my_fields,my_id):
    ...
    class square():
        ...
        def clear(self):
            self.lenth=0
            self.direc = 'U'
        def goAhead(self,walk_lenth,size,my_fields,my_id):
            '''
            walk_lenth：要前进的距离
            '''
            if walk_lenth>0:
                ...
                #此处写入判断旋转方向的函数
                storage['turn'] = turnDir(size,my_fields,my_id)
